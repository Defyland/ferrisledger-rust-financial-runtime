...
| Benchmarks have scripts and measured baseline | `benchmarks/*.js`, `crates/ferrisledger-runtime/benches/replay.rs`, `benchmarks/results/2026-05-30-smoke.md`, `benchmarks/results/2026-05-31-load.md` | Done | Smoke, load, and Criterion results are recorded; stress/spike scripts are inspectable. |
| CPU and memory notes are measured under load | `benchmarks/results/2026-05-31-load.md` | Done | Load baseline records latency, throughput, error rate, server CPU, server RSS, and k6 memory. |
...
| Local coverage command runs | `docs/spec-driven/verification-report.md` | Done | Homebrew LLVM binaries are supplied through `LLVM_COV` and `LLVM_PROFDATA`; local coverage report runs. |
